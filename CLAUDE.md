...
- Три формата строк:
  ...
  - `autolearn=true` — настройка (тихое обучение без подтверждения)
- Возвращает `ConfigData(browsers, patterns, autolearn)`
...
3. Определяет тип строки:
   - Содержит `=` и ключ-слово настройки (например `autolearn`) → настройка
   - Содержит `=` и цифровой ключ → браузер
